...
def main():
    ...
    for idx in tqdm(range(cfg.NUM_TO_DRAW)):
        ...
        reshape(draw_info_dict, keys_to_process[0:2], 'b h w c -> (b h) w c', should_log=cfg.SHOULD_LOG)
        reshape(draw_info_dict, keys_to_process[2:], 'c h w -> h w c', should_log=cfg.SHOULD_LOG)
